...
const Navbar = () => {
  ...
  return (
    <nav className="bg-gray-900 text-white p-4 relative flex justify-center z-50">
      <ul className="flex space-x-6">
        ...
        <li
          ...
          onMouseLeave={() => setTimeout(() => setShowMegaMenu(false), 300)}
        >
          <span className="cursor-pointer hover:text-gray-400">Services</span>
          <div
            className={`absolute left-0 mt-2 w-full md:w-[700px] bg-white text-black p-6 shadow-lg grid grid-cols-1 md:grid-cols-2 gap-6 transition-opacity duration-300 ${showMegaMenu ? 'opacity-100 visible' : 'opacity-0 invisible'}`}
            onMouseEnter={() => setShowMegaMenu(true)}
            onMouseLeave={() => setTimeout(() => setShowMegaMenu(false), 300)}
          >
